...
static void connectToWiFi()
{
  ...
  while (WiFi.softAPgetStationNum() == 0) {
    delay(1000);
    Serial.println("Waiting for station to connect...");
  }
  ...
  while (WiFi.softAPgetStationNum() > 0) {
    ...
    delay(1000);
    Serial.println("Waiting for data from station... IP: " + WiFi.softAPIP());
...
static void sendTelemetry()
{
  ...
  mqtt_client.publish(telemetry_topic, normalReading().c_str(), false);
  Serial.println("OK");
  delay(100);
  digitalWrite(LED_PIN, LOW);
}
...
String normalReading()
{
  //Serial.print("\n---------------------------------NORMAL READING---------------------------------------");
  incomingByte = 0;

  while(incomingByte != 0x42){
    delay(1);
    incomingByte = Serial1.read();
  }

  Serial1.readBytes(message, 31);
  pm25[0] = message[6];
  pm25[1] = message[7];
  pm10[0] = message[8];
  pm10[1] = message[9];
  int inputPm25 = pm25[0] | (pm25[1]<<8);
  int inputPm10 = pm10[0] | (pm10[1]<<8);

  //Serial.print("\nPM 2.5: ");
  //Serial.print(inputPm25);
  //Serial.print("\nPM 10: ");
  //Serial.print(inputPm10);
  //Serial.print("\nFull message: ");
  //for(int i=0; i<31; i++){
  //  Serial1.print(" ");
